var StrongConnectionPolicy =
    draw2d.policy.connection.ConnectionCreatePolicy.extend({

      NAME: "draw2d.policy.connection.StrongConnectionPolicy",
       ...
       * @param {Boolean} shiftKey true if the shift key has been pressed during
       * this event
       * @param {Boolean} ctrlKey true if the ctrl key has been pressed during
       * the event
       */
      onMouseDown: function(canvas, x, y, shiftKey, ctrlKey) {
        ...
        // this can happen if the user release the mouse button outside the
        ...
        if (port.isDraggable()) {
          var canDragStart = port.onDragStart(
              x - port.getAbsoluteX(), y - port.getAbsoluteY(), shiftKey,
              ctrlKey);
          if (canDragStart) {
       ...
       * @param {Number} dx2 The x diff since the last call of this dragging
       * operation
       * @param {Number} dy2 The y diff since the last call of this dragging
       * operation
       * @template
       */
      onMouseDrag: function(canvas, dx, dy, dx2, dy2, shiftKey, ctrlKey) {
        try {
          if (this.mouseDraggingElement !== null) {
            ...
            var target =
                canvas.getBestFigure(de.getAbsoluteX(), de.getAbsoluteY(), de);
            ...
            var target =
                canvas.getBestFigure(p.x, p.y, this.mouseDraggingElement);
            ...
            if (target !== this.currentDropTarget) {
              if (this.currentDropTarget !== null) {
                ...
                this.currentDropTarget.fireEvent(
                    "dragLeave", {draggingElement: this.mouseDraggingElement});
                this.currentDropTarget = null;
              }
              if (target !== null) {
                this.currentDropTarget =
                    target.delegateTarget(this.mouseDraggingElement);
                // inform all listener that the element has accept the dragEnter
                // event
                //
                if (this.currentDropTarget !== null) {
      ...
      onMouseUp: function(canvas, x, y, shiftKey, ctrlKey) {
        if (this.mouseDraggingElement !== null) {
          ...
          // @since 5.3.3
          de.fireEvent(
              "dragend", {x: x, y: y, shiftKey: shiftKey, ctrlKey: ctrlKey});
