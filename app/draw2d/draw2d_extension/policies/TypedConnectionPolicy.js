...
angular
    ...
    .module('draw2d.policies.TypedConnectionPolicy', [])

    .factory('TypedConnectionPolicy', [
      '$rootScope',
      function($rootScope) {

        var TypedConnectionPolicy =
            draw2d.policy.connection.ConnectionCreatePolicy.extend({

              NAME: "draw2d.policy.connection.TypedConnectionPolicy",
               ...
               * @param {Boolean} shiftKey true if the shift key has been
               * pressed during
               * this event
               * @param {Boolean} ctrlKey true if the ctrl key has been pressed
               * during
               * the event
               */
              onMouseDown: function(canvas, x, y, shiftKey, ctrlKey) {
                ...
                // in this case the ResizeHandle has prio. and handled by
                // another
                // Policy
                ...
                // this can happen if the user release the mouse button outside
                // the
                // window during a drag&drop
                // operation. In this case we must fire the "onDragEnd" event
                // postpond.
                //
               ...
               *
               * @param {draw2d.Canvas} canvas
               * @param {Number} dx The x diff between start of dragging and
               * this event
               * @param {Number} dy The y diff between start of dragging and
               * this event
               * @param {Number} dx2 The x diff since the last call of this
               * dragging
               * operation
               * @param {Number} dy2 The y diff since the last call of this
               * dragging
               * operation
               * @template
               */
              onMouseDrag: function(
                  canvas, dx, dy, dx2, dy2, shiftKey, ctrlKey) {
                try {
                  if (this.mouseDraggingElement !== null) {
                    ...
                    var target = canvas.getBestFigure(
                        de.getAbsoluteX(), de.getAbsoluteY(), de);
                    ...
                    if (target !== ct) {
                      if (ct !== null) {
                        ...
                        de.editPolicy.each(function(i, e) {
                          if (e instanceof
                              draw2d.policy.port.PortFeedbackPolicy) {
                    ...
                    var target = canvas.getBestFigure(
                        p.x, p.y, this.mouseDraggingElement);

                    if (target !== this.currentDropTarget) {
                      if (this.currentDropTarget !== null) {
                        this.currentDropTarget.onDragLeave(
                            this.mouseDraggingElement);
                        this.currentDropTarget.fireEvent(
                            "dragLeave",
                      ...
                      if (target !== null) {
                        this.currentDropTarget =
                            target.delegateTarget(this.mouseDraggingElement);
                        // inform all listener that the element has accept the
                        // dragEnter
                        // event
                        //
                        if (this.currentDropTarget !== null) {
                          this.currentDropTarget.onDragEnter(
                              this.mouseDraggingElement);  // legacy
                          this.currentDropTarget.fireEvent(
                              "dragEnter",
                              {draggingElement: this.mouseDraggingElement});
                        }
              ...
              onMouseUp: function(canvas, x, y, shiftKey, ctrlKey) {
                if (this.mouseDraggingElement !== null) {
                  ...
                  de.fireEvent(
                      "dragend",
                      {x: x, y: y, shiftKey: shiftKey, ctrlKey: ctrlKey});
                  ...
                  if (this.currentDropTarget !== null) {
                    this.mouseDraggingElement.onDrop(
                        this.currentDropTarget, x, y, shiftKey, ctrlKey);
                    ...
                    if (this.currentDropTarget instanceof draw2d.Port) {
                      ...
                      try {
                        if (request.source.userData.datatype !=
                            request.target.userData.datatype) {
                          command = null;
                          console.log("CONNECTION ERROR", $rootScope);
                        }
                      } catch (e) {
                        command = null;
                      }
                      ...
                      if (command !== null) {
                        ...
                        this.currentDropTarget.onCatch(
                            this.mouseDraggingElement, x, y, shiftKey, ctrlKey);
                      }
                    }
                  }
